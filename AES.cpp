...
AESModeOfOperation::AESModeOfOperation(AESMode_t mode, unsigned char * key, unsigned char * iv)
	:m_mode(mode)
{
	m_aes = new AES(m_key);
	assert(m_aes != NULL);
...
int AESModeOfOperation::EncryptInternal(const unsigned char *_in, size_t _length, unsigned char *_out) {
	...
	unsigned char* cipherout = _out;
	memset(cipherout, 0, _length);
...
int AESModeOfOperation::DecryptInternal(const unsigned char *_in, size_t _length, unsigned char *_out) {
	// TODO :
	bool first_round = true;
	int rounds = 0;
	unsigned char ciphertext[16] = { 0 };
	...
	unsigned char* plainout = _out;
	memset(plainout, 0, _length);
...
#define PADDING_SIZE(len) ((len-1)/16 * 16 + 16)

std::string AESModeOfOperation::Encrypt(const std::string & input)
{
	bool padding = m_mode == MODE_ECB || m_mode == MODE_CBC;
	size_t len = input.size();

	std::string src = input;

	if (padding) {
		src.resize(PADDING_SIZE(len), 0);
	}

	std::string dst(src.size(), 0);
	EncryptInternal((const unsigned char*)src.c_str(), src.size(), (unsigned char*)dst.c_str());
	return dst;
}

std::string AESModeOfOperation::Decrypt(const std::string & input)
{
	bool padding = m_mode == MODE_ECB || m_mode == MODE_CBC;
	if (padding && input.size() % 16 != 0) {
		return std::string();
	}

	std::string dst(input.size(), 0);
	DecryptInternal((const unsigned char*)input.c_str(), input.size(), (unsigned char*)dst.c_str());
	return dst;
}

std::string AESModeOfOperation::EncryptBase64(const std::string & input)
{
	std::string dest = Encrypt(input);
	return Base64::EncodeString(dest);
}

std::string AESModeOfOperation::DecryptBase64(const std::string & input)
{
	std::string dest;
	if (!Base64::DecodeString(input, dest)) {
		return std::string();
	}
	return Decrypt(dest);
}
